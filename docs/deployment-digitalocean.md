...
This guide walks you through deploying **Fuorix** so that:
...
5. Set a **cluster name** (e.g. `fuorix-db`).
...
2. Add a database, e.g. `fuorix`.
3. Use it in the connection string: `.../fuorix?sslmode=require`.
...
Set `ADMIN_EMAIL` / `ADMIN_PASSWORD` env vars for the seed job; if `ADMIN_PASSWORD` is unset, the seed prints a generated password once. Change the password after first login.
...
```bash
cd /opt
git clone https://github.com/YOUR_ORG/fuorix.git
cd fuorix/backend
python3 -m venv .venv
...
Use a process manager (systemd) so it restarts on reboot. Example unit `/etc/systemd/system/fuorix-backend.service`:
...
[Service]
User=root
WorkingDirectory=/opt/fuorix/backend
Environment="PATH=/opt/fuorix/backend/.venv/bin"
ExecStart=/opt/fuorix/backend/.venv/bin/gunicorn app.main:app -w 2 -k uvicorn.workers.UvicornWorker -b 0.0.0.0:8000
Restart=always
...
systemctl enable fuorix-backend
systemctl start fuorix-backend
```
...
1. **Code changes**: Push to the connected branch; App Platform will redeploy. On a Droplet, `git pull` in `/opt/fuorix`, then restart the service.
