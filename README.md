...
5. **After the booking:** invitees get reminder emails 24h and 1h before the call (with the Meet link and a reschedule link). If a host moves or deletes a booking inside the app, the Google event is updated/removed and the invitee is emailed. When Google Calendar fails for a host, bookings still go through (with a plain .ics) and admins + the host are notified and emailed, at most once an hour. Once the call has happened, record the outcome on the meeting (completed / no-show + note) and optionally move the lead's stage in the same step. Pages can have **co-hosts** (slots are the union of everyone's free time; the least-loaded free host takes the booking) and **date overrides** (close a day or set special hours). Bookings capture `utm_*` / referrer / landing page from the website for attribution, disposable email domains are rejected, and domains that cannot receive mail are refused.
6. **Google Calendar + Meet (recommended):** when the host has connected Google in **Profile**, each booking is created on their Google Calendar with an auto-generated **Google Meet** link (stored as the meeting location and shown in the confirmation), Google emails the invitation to the invitee, reschedules and cancellations are pushed through, and the host's Google busy times block slots on the public page. Setup: in Google Cloud Console create a project, enable the *Google Calendar API*, configure the OAuth consent screen (External; add each host's Gmail as a test user, or publish the app so refresh tokens do not expire after 7 days), create an *OAuth client ID* of type *Web application* with redirect URI `https://app.yourdomain.com/api/v1/integrations/google/callback`, then set `GOOGLE_CLIENT_ID` / `GOOGLE_CLIENT_SECRET` in `backend/.env` and restart. Tokens are stored encrypted with the vault key.
